describe('ep_script_elements - calculate scene length', function() {
  ...
  context('when there are more than one user on the pad edited', function() {
    before(function(done) {
      utils.speedUpTests();
...
ep_script_elements_test_helper.calculateSceneLength = {
  ...
  getExpectedScenesLength: function() {
    ...
    return _.map(
      this.SCENE_LINES_LENGTH,
      function(sceneLineLength) {
  ...
  testIfScenesLengthValueIsCorrect: function() {
    ...
    _.each(
      scenesLength,
      function(sceneLength, index) {
        var expectedSceneLength = expectedScenesLength[index];
        expect(sceneLength).to.be.within(expectedSceneLength - this.TOLERANCE, expectedSceneLength + this.TOLERANCE);
      },
      this
    );
  },
  ...
  testSceneLenghtWasNotUpdated: function(targetScene, originalSceneId, done) {
    ...
    this.waitForSceneToBeUpdated(targetScene, originalSceneId, function() {
      ...
      expect().fail(function() {
        return `Length of scene ${targetScene} was updated`;
      });
    }).fail(function() {
    ...
    this.waitForSceneToBeUpdated(targetScene, originalSceneId, function() {
      helper
        ...
        .done(function() {
          ...
          expect().fail(function() {
            return `Caret moved from line of scene ${targetScene}`;
          });
        })
        .fail(function() {
